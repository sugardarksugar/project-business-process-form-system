...
function setupSqlite(options) {
    ...
    (0, file_1.addDependencies)('better-sqlite3-schema', '^2.3.5');
    (0, file_1.addDependencies)('@types/integer', '^4.0.1', 'dev');
    (0, file_1.addDependencies)('better-sqlite3-proxy', '^1.5.0');
    const code = `
import { toSafeMode, newDB } from 'better-sqlite3-schema'

export const dbFile = ${(0, util_1.inspect)(options.dbFile)}
...
function setupEnvFile(options) {
    ...
    (0, file_1.addDependencies)('dotenv', '^16.0.1');
    (0, file_1.addDependencies)('populate-env', '^2.0.0');
    const code = `
...
function setupKnexFile(options) {
    ...
    (0, file_1.addDependencies)('knex', '^2.3.0');
...
function generateAutoMigrate(options) {
    ...
    (0, sort_tables_1.sortTables)(options.parsed_table_list).forEach(table => {
        ...
        if (!existing_table) {
            up_lines.push((0, text_to_knex_1.toKnexCreateTableCode)(table));
        ...
        function addDropColumn(field, options) {
            if (is_sqlite) {
                ...
                options.raw_drop_lines.unshift(`  await knex.raw(${(0, util_1.inspect)(`alter table ${table} drop column ${col}`)})`);
                if (is_unique) {
                ...
                options.table_add_lines.push((0, text_to_knex_1.toKnexCreateColumnCode)(field));
...
function alterType(field) {
    let code = 'table';
    code += (0, text_to_knex_1.toKnexCreateColumnTypeCode)(field);
    if (field.is_null) {
        code += '.nullable()';
    }
    else {
...
function loadTableList(knex, db_client) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (db_client === 'pg' || db_client.includes('postgres')) {
            return yield (0, pg_to_text_1.scanPGTableSchema)(knex);
        }
        if (db_client.includes('mysql')) {
            return yield (0, mysql_to_text_1.scanMysqlTableSchema)(knex);
        }
