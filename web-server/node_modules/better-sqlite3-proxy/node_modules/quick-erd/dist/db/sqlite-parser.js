...
exports.parseCreateTable = exports.parseTableSchema = void 0;
const enum_1 = require("../core/enum");
function parseTableSchema(rows) {
    const table_rows = rows.filter(row => row.type === 'table');
    ...
    for (const index_row of index_rows) {
        ...
        const table = table_list.find(table => table.name === (index === null || index === void 0 ? void 0 : index.table));
        const field = table === null || table === void 0 ? void 0 : table.field_list.find(field => field.name === (index === null || index === void 0 ? void 0 : index.field));
        if ((index === null || index === void 0 ? void 0 : index.is_unique) && field) {
...
function parseCreateTable(sql) {
    ...
    parseCreateColumns(sql).forEach(part => {
        ...
        if (lower.match(/check.*in.*/)) {
            let match = ((_a = lower.match(/check(.*)/)) === null || _a === void 0 ? void 0 : _a[1].trim()) || '';
            if (match.startsWith('(') && match.endsWith(')')) {
                match = match.slice(1, match.length - 1);
            }
            const matches = match.match(/`?(.*?)`?\s*in.*\((.*)\)/);
            if ((matches === null || matches === void 0 ? void 0 : matches[1]) === name) {
                type = 'enum(' + matches[2] + ')';
...
function parseCreateIndex(sql) {
    ...
    const match = sql.match(/create unique index .* on \`?(.*?)\`? \(\`?(.*?)\`?\)/i);
    if (!match)
        return null;
    const table = match[1];
    const field = match[2];
