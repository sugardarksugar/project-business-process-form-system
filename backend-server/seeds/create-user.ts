...
export async function seed(knex: Knex): Promise<void> {
    ...
    for (let user of users) {
        let row = await knex('user').select('id').where({ email: user.email }).first()
        if (row) {
    ...
    for (let form_i_Field of form_i_Fields) {
        let row = await knex('field').select('id')
            .where({
    ...
    for (let form_ii_Field of form_ii_Fields) {
        ...
        if (row) {
            await knex('field')
                .where({ id: row.id })
                .update(form_ii_Field)
        } else {
            await knex("field")
                .insert(form_ii_Field);
        }
    }
    ...
    for (let template of templates) {
        let row = await knex('template').select('id')
            .where({
                name: template.name
            })
            .first()
